...
class SimilarityService:
    ...
    def _process_queue(self):
        with self.task_lock:
            if self.running_tasks >= self.max_concurrent_tasks or not self.task_queue:
                return
    ...
    def _analyze_task(self, task_id: str, tender_file_path: str, bid_file_paths: List[str]):
        try:
            self.tasks[task_id]["status"] = "running"
            # 1. 招标文件分段并向量化
            tender_segments = self._extract_and_segment(tender_file_path)
            tender_texts = [seg['text'] for seg in tender_segments]
            tender_vecs = self._encode_texts(tender_texts) if tender_texts else np.zeros((1, 384))
            ...
            for bid_path in bid_file_paths:
                ...
                vecs = self._encode_texts(texts) if texts else np.zeros((1, 384))
            ...
            for segs, vecs in zip(bid_segments_list, bid_vecs_list):
                ...
                for i, v in enumerate(vecs):
                    if tender_vecs.shape[0] > 0:
                        sims = np.dot(tender_vecs, v) / (np.linalg.norm(tender_vecs, axis=1) * np.linalg.norm(v) + 1e-8)
                        if np.max(sims) < 0.8:
                            keep_idx.append(i)
                    else:
                        keep_idx.append(i)
            ...
            for i in range(len(filtered_bid_segments)):
                for j in range(i+1, len(filtered_bid_segments)):
                    ...
                    for idx_i, v_i in enumerate(vecs_i):
                        sims = np.dot(vecs_j, v_i) / (np.linalg.norm(vecs_j, axis=1) * np.linalg.norm(v_i) + 1e-8)
