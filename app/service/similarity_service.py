...
if __name__ == "__main__":
    # 配置日志格式
    logging.basicConfig(level=getattr(logging, default_config.LOG_LEVEL), format='%(asctime)s %(levelname)s %(message)s')
...
class SimilarityService:
    def __init__(self, config=None):
        ...
        self.table_processing_mode = self.config.TABLE_PROCESSING_MODE if self.config.TABLE_PROCESSING_MODE in ["cell", "row"] else "row"
        ...
        # 预计算表格相似度阈值（提高区分度）
        self.table_row_threshold = self.bid_similarity_threshold + 0.15
        self.table_cell_threshold = self.bid_similarity_threshold + 0.1
                        ...
                        for idx_b in range(len(batch_vecs)):
                            for top_k in range(self.similarity_top_k):
                                ...
                                if (is_table_row_i and is_table_row_j) or (is_table_cell_only_i and is_table_cell_only_j):
                                    row_i = segs_i[idx_i].get('row')
                                    row_j = segs_j[max_idx].get('row')
                                    table_i = segs_i[idx_i].get('table_idx')
                                    table_j = segs_j[max_idx].get('table_idx')
                                    row_match = row_i == row_j
                                    table_match = table_i == table_j
                                    if is_table_cell_only_i and is_table_cell_only_j:
                                        col_i = segs_i[idx_i].get('col')
                                        col_j = segs_j[max_idx].get('col')
                                ...
                                if is_valid and sim > 0.99:
                                    # 检查是否包含大量相同的专业术语或固定表述
                                    common_terms = ['投标函', '投标保证金', '工程总承包', '招标文件', '承诺函']
                                ...
                                if is_valid:
                                    ...
                                    if not stopword_evade and sim < 0.95:
                                        # 简单同义词替换检测逻辑
                                        from app.service.text_utils import is_synonym_evade
                                        synonym_evade = is_synonym_evade(text1, text2)
                                    
                                    # 新增：检测语义保持但词汇变化较大的情况
                                    semantic_evade = False
                                    if sim > 0.85 and sim < 0.92 and not order_changed and not stopword_evade and not synonym_evade:
                                        # 可能存在更高级的规避行为
                                        semantic_evade = True
                                    # 检查是否为表格元素
                                    is_table_cell_i = segs_i[idx_i].get('is_table_cell', False)
                                    ...
                                    if is_table_cell_i:
                                        table_info = {
                                            'row': segs_i[idx_i].get('row'),
                                            'table_idx': segs_i[idx_i].get('table_idx')
                                        }
                                        # 只有按单元格处理时才添加列信息
                                        if 'col' in segs_i[idx_i]:
                                            table_info['col'] = segs_i[idx_i].get('col')
                                        detail.update(table_info)
                                    if is_table_cell_j:
                                        similar_table_info = {
                                            'similar_row': segs_j[max_idx].get('row'),
                                            'similar_table_idx': segs_j[max_idx].get('table_idx')
                                        }
                                        # 只有按单元格处理时才添加列信息
                                        if 'col' in segs_j[max_idx]:
                                            similar_table_info['similar_col'] = segs_j[max_idx].get('col')
